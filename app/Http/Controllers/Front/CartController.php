...
class CartController extends FrontBaseController
{
    ...
    public function addnumcart(Request $request)
    {
        ...
        $cart->addnum($prod, $prod->id, $qty, $size,$color,$order_type,$size_qty,$size_price,$size_key,$keys,$values,$affilate_user);
    ...
    public function addservicetonumcart(Request $request)
    {
        ...
        $curr = $this->curr;
        $size_price = ($size_price / $curr->value);
        $prod = UserService::where('id','=',$id)->first(['id','user_id','slug','name','photo','price','file']);
        if($prod->type != 'Physical'){
            $qty = 1;
        }
